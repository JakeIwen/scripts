#!/bin/bash
# Fail closed unless a disk-backed share path is the exact labeled filesystem.
...
disk_policy_samba_share_name "$label" >/dev/null || {
  echo "Samba mount gate: unsupported label" >&2
  exit 2
}

if [[ "$require_block_device" != 0 && "$require_block_device" != 1 ]]; then
  echo "Samba mount gate: invalid block-device setting" >&2
  exit 2
fi
...
if [[ ! -e "$label_link" ]]; then
  echo "Samba mount gate: exact label $label is unavailable" >&2
  exit 1
fi
...
if (( findmnt_status != 0 )) || [[ -z "$source_output" ]]; then
  echo "Samba mount gate: $target is not an exact mount" >&2
  exit 1
fi
source_count=$(printf '%s\n' "$source_output" |
  /usr/bin/awk 'NF { count++ } END { print count + 0 }')
if (( source_count != 1 )); then
  echo "Samba mount gate: $target has ambiguous mount sources" >&2
  exit 1
fi
source_path=$(printf '%s\n' "$source_output" | /usr/bin/awk 'NF { print; exit }')

resolved_source=$("$readlink_command" -f -- "$source_path" 2>/dev/null) || {
  echo "Samba mount gate: cannot resolve mounted source" >&2
  exit 1
}
resolved_label=$("$readlink_command" -f -- "$label_link" 2>/dev/null) || {
  echo "Samba mount gate: cannot resolve exact label $label" >&2
  exit 1
}
if [[ -z "$resolved_source" || "$resolved_source" != "$resolved_label" ]]; then
  echo "Samba mount gate: $target is not backed by exact label $label" >&2
  exit 1
fi
if (( require_block_device )) && [[ ! -b "$resolved_source" ]]; then
  echo "Samba mount gate: resolved source is not a block device" >&2
  exit 1
fi
