...
[[ "$(row_count "$display_power_uuid")" == 1 ]] || fail "expected exactly one Display Power record"
...
[[ "$duplicate_parent" == "$media_pk" ]] || fail "duplicate Media is not nested under the expected Media record; refusing an unverified repair"
...
-- This valid item was detached by the failed imports. Restore it at the end.
UPDATE ZBTTBASEENTITY
