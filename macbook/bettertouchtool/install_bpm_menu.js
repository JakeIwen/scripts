...
function matches(actual, expected) {
    ...
    return actions.length === 1 && enabled(actions[0]) && Number(actions[0].BTTPredefinedActionType) === 206 &&
        ...
        (actual.BTTMenuConfig || {}).BTTMenuElementIdentifier === 'bpm-over-time';
}
...
function run(argv) {
    ...
    if ((previous && Number(previous.BTTTriggerType) !== 773) || tools.BTTMenuItems.some(x =>
        x.BTTUUID !== desired.BTTUUID && (x.BTTMenuName === 'BPM Over Time' ||
                                        (x.BTTMenuConfig || {}).BTTMenuElementIdentifier === 'bpm-over-time'))) {
        throw new Error('Conflicting BPM Over Time item; no changes made.');
    }
    if (argv[2] === 'inspect') {
    ...
    if (matches(previous, desired)) return 'Already configured: Media > Tools > BPM Over Time.';
    const manager = $.NSFileManager.defaultManager;
    const directory = argv[1] + '/btt-before-bpm-over-time-' + ObjC.unwrap($.NSUUID.UUID.UUIDString);
    ...
    return 'Verified configuration: Media > Tools > BPM Over Time. Backup: ' + backup;
}
