...
execFileSync('/usr/bin/python3', [path.join(folder, 'install_convert_video_menu.py')]);
const payload = JSON.parse(fs.readFileSync(path.join(__dirname, '../build/Convert Video Tools.json')));
const code = fs.readFileSync(path.join(folder, 'install_convert_video_menu.js'), 'utf8');
...
function setup({backupFails = false, badReadback = false, race = false} = {}) {
    ...
    const btt = {
        ...
        add_new_trigger(json,{parent_uuid}) {
            ...
            const item=JSON.parse(json);item.BTTOrder=8;
...
test('detects failed readback and name collisions rather than claiming success',()=>{
    ...
    const collision=setup();collision.tools.BTTMenuItems.push({BTTUUID:'foreign',BTTMenuName:'Convert Video'});
